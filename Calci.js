...
const logger = winston.createLogger({
  ...
  try{
    if(isNaN(n1)) {
      logger.error("n1 data is not correct");
      throw new Error("n1 data is not correct");
    }
    if(isNaN(n2)) {
        logger.error("n2 data is not correct");
        throw new Error("n2 data is not correct");
    }
    if (n1 === NaN || n2 === NaN) {
      console.log()
    ...
    flag = true; 
    logger.info(`${n1} and ${n2} are valid`);
    return flag;
    } catch(error) {
    console.error(error)
...
app.get("/add", (req,res)=>{
    try{
      ...
      logger.info(`Received ${n1} and ${n2} parameters for addition`);
...
app.get("/sub", (req,res)=>{
    try{
     const n1= parseFloat(req.query.n1); 
     const n2= parseFloat(req.query.n2);
     logger.info(`Received ${n1} and ${n2} parameters for substraction`);
     const flag = checkNumbers(n1,n2,req,res);
     if(flag == true)
     {
...
app.get("/mul", (req,res)=>{
  try{
    const n1= parseFloat(req.query.n1); 
    const n2= parseFloat(req.query.n2);
    logger.info(`Received ${n1} and ${n2} parameters for multiplication`);
    const flag = checkNumbers(n1,n2,req,res);
    if(flag == true)
    {
...
app.get("/div", (req,res)=>{
    try{
     const n1= parseFloat(req.query.n1); 
     const n2= parseFloat(req.query.n2);
     logger.info(`Received ${n1} and ${n2} parameters for division`);
     const flag = checkNumbers(n1,n2,req,res);
     if(flag == true)
     {
...
function handleSqrt(req, res) {
    try {
        const n1 = parseFloat(req.query.n1);
        logger.info(`Received ${n1} parameter for square root`);
...
app.get("/mod", (req, res) => {
    try {
        const n1 = parseFloat(req.query.n1);
        const n2 = parseFloat(req.query.n2);
        logger.info(`Received ${n1} and ${n2} parameters for modulo`);
        if (checkNumbers(n1, n2, req, res)) {
